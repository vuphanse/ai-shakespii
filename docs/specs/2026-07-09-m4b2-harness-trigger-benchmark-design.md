...
- Exit codes: **0** benchmark completed (regardless of deltas — bench measures, it does not gate); **1** any run failed after its retry — executor half gets one retry, grader half keeps the shared M4b-1 budget; full failure contract in §8.1 (partial artifacts kept, nothing cached for the failed run, no `benchmark.json` written); **2** usage / invalid suite / claude unavailable.
...
Run-failure contract (both halves of a sample):

- **Executor failure**: statuses `timeout`, `nonzero-exit`, and `no-result` (runner reports `completed` but the stream carries no result event) — exactly the M4b-1 classification in `llm-stages.ts`. One retry re-issuing the identical request after restaging the run dir (same single-retry rule as trigger reps, §7.1; the retry's staging overwrites the failed attempt's artifacts). Second failure → the run is **failed**: the final attempt's artifacts (`events.jsonl`, `transcript.md`, `outputs/metrics.json`) are kept on disk for diagnosis, grading is not attempted, and no `grading.json` is ever written — which is what "uncached" means, since the cache probe requires a valid `grading.json`.
- **Grader failure**: grading runs only after executor success; a run failing after the grader's shared retry budget (M4b-1 semantics unchanged) is failed the same way — executor artifacts kept, no valid `grading.json`, uncached.
- **Abort**: the first failed run aborts bench immediately — no further sessions are spawned and no `benchmark.json` (not even a partial one) is written. Fail-fast here is deliberate and NOT the test-stage behavior (test stages continue and accumulate findings): `benchmark.json` requires the complete run matrix for its stats, so once a run is unrecoverable the document is unwritable and every further session would spend tokens on a doomed suite.
- **Output on failure** (exact, testable): message `bench run failed (eval <id>, <config>, run <n>): <detail>`, where `<detail>` is `executor <status> — <errorMessage or 'no result event'>` for executor failures (the M4b-1 finding text) or the grader's failure string verbatim. Pretty mode prints the message as the only output line; `--json` prints `{"error": "<message>"}` (single-key object). Exit 1 in both modes.
...
- Bench: full matrix ordering (runner receives with_skill run 1..N then without_skill per eval, ascending); prompt shapes (preamble present/absent); stats against hand-computed fixtures (incl. n=1 → stddev 0); golden `benchmark.json` byte-compared AND passed through `validateBenchmarkJson`; delta signs (positive, negative, zero → `+0.00`); replay byte-identical at zero runner calls; deterministic gate → exit 2, zero runner calls.
- Bench run-failure contract (§8.1): executor `timeout` on run k → exactly one retry issued, then abort — pinned failure message bytes in pretty mode and `{"error": "<message>"}` in `--json`, exit 1, no `benchmark.json`, final attempt's artifacts on disk, no `grading.json`, zero runner calls after the failed run; `no-result` (completed, no result event) classified and retried identically; grader-exhaustion failure aborts with the grader's failure string verbatim; re-run after a failed suite re-executes the failed sample (nothing was cached for it) while earlier successful runs replay from cache.
- Hardening (§9): deterministic finding `skill_name must be a safe path segment` for `../x`, `a/b`, `a\b`, `.`, and `..` (and a clean pass for a `dot.dash-name`-style safe segment); run-dir helper throws on a separator-bearing name (defense-in-depth guard); grader-retry observability — a scripted gate-failure-then-success grading yields `grading.json` `timing.grader_retries` / `timing.grader_retry_causes` with asserted values, a first-try success omits both keys, and a cached pre-observability document (no such keys) still replays as a cache hit.
