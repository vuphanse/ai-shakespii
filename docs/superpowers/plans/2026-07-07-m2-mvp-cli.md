...
**Files:**
- Create: `src/cli/format/pretty.ts`, `src/cli/format/json.ts`, fixture `tests/fixtures/warn-only/SKILL.md`
...
- [ ] **Step 1: Create the warn-only fixture and write the failing test**

The warning-only exit-0 contract (spec §4: exit 0 = no error-severity findings, warnings allowed) needs a fixture whose ONLY finding is a warn. `tests/fixtures/warn-only/SKILL.md` — the unknown field trips FM01's warn override while every error-severity seed rule passes (trigger-first description, name = dir, worked Examples pair, no links, no tokens):
...
Note: `fm01-unknown-field` cannot serve as the warn-only case — it lacks an Examples section, so CT03 adds an error and the exit code is 1. That is exactly why the dedicated `warn-only` fixture above exists: it pins the spec §4 "exit 0 with warnings allowed" contract unconditionally, in both output modes.
...
```bash
git add src/cli/lint.ts src/cli/format/pretty.ts src/cli/format/json.ts tests/cli/lint.test.ts tests/fixtures/warn-only
...
**Interfaces:**
- Consumes: the real CLI via `lint --json` per skill (spec §5 protocol — calibration exercises the CLI/JSON contract, not lib internals). Read-only sweep; never edits corpus skills.
...
cd "$WORK"
shakespii --version
shakespii lint /Users/vuphan/Dev/ai-shakespii/tests/fixtures/minimal-pass
shakespii init linked-demo
grep -c 'name: linked-demo' linked-demo/SKILL.md
shakespii lint linked-demo --json > lint.json; echo "exit=$?"
cd -
```
Expected: `0.2.0`; `✔ 0 problems`; the scaffold success message; `1` from the grep (template resolved from the package root and `{{name}}` substituted); `exit=1` (the fresh scaffold is lint-RED). This proves BOTH profile resolution (`lint`) and template resolution (`init`) from an arbitrary cwd through the linked binary. (If `shakespii` is not found, use `$HOME/.bun/bin/shakespii` explicitly and note that PATH needs `~/.bun/bin`.)
