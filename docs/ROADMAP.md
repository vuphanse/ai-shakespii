...
- [x] Eval-fix loop (user-approved same day): fixture-grounded scenario evals + trigger slash-form replacement — `shakespii test --run --triggers` on ai-whisper-workflow exits 0 end to end (grading 0 findings, triggers 19/20 = 0.95); surfaced two harness-backlog findings (scenario stage cannot host near-miss negatives — executor prompt frame; whole-dir cache key makes eval edits re-buy the trigger matrix) — CALIBRATION-M5D "Follow-up loop"
- [ ] Parked (need user approval each): ai-whisper-workflow description scope clause for the noisy dashboard-debugging near-miss (borderline across all three sweeps); optional live sweeps for the other six derived skills; optional remote for ~/Dev/ai-skills
